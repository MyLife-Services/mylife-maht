...
export function setupMcpManager(): McpManager {
  ...
  const discoverTools = async (sessionId: string): Promise<ToolInfo[]> => {
    ...
    const modifiedTools = tools.map(toolInfo => {
      ...
      if (serverId && toolInfo.credentialRequirements && toolInfo.credentialRequirements.length > 0) {
  ...
  const executeToolCall = async (
    ...
    if (rateLimit && rateLimit.queue.length >= RATE_LIMIT_CONFIG.maxQueueLength) {
      return {
        content: [
          {
            type: "text",
