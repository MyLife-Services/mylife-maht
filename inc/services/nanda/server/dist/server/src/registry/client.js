        ...
        try {
            ...
            if (axios.isAxiosError(error) && error.response?.status === 429) {
                ...
                if (error.response.headers['retry-after']) {
                    ...
                    retryAfter = Math.min(globalRateLimitState.rateLimitBackoff * Math.pow(2, globalRateLimitState.consecutiveErrors - 1), 60 * 60 * 1000);
                }
...
export class RegistryClient {
    ...
    async getServerDetails(serverId) {
        try {
            ...
            const response = await axios.get(`${this.baseUrl}/api/v1/servers/${serverId}/`, {
                headers,
            });
            return response.data;
        }
