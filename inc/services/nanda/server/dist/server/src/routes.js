...
async function getWeightedRatingScore(serverId) {
    try {
        ...
        if (axios.isAxiosError(error) && error.response?.status === 429) {
            ...
            const backoffTime = Math.min(ratingApiState.rateLimitBackoff * Math.pow(2, ratingApiState.consecutiveErrors - 1), 60 * 60 * 1000);
...
export function setupRoutes(app, mcpManager) {
    ...
    app.get("/api/tools/credentials", (req, res) => {
    ...
    app.post("/api/tools/credentials", async (req, res) => {
