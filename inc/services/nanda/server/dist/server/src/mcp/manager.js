...
export function setupMcpManager(io) {
    ...
    const registerServer = async (serverConfig) => {
        try {
            ...
            toolRegistry.registerTools(serverConfig.id, serverConfig.name, serverConfig.rating ?? 0, client, toolsResult.tools);
            // Store the connected client for later use
            connectedClients.set(serverConfig.id, client);
            console.log(`Registered server ${serverConfig.name} with ${toolsResult?.tools?.length} tools`);
            // Successful Registration
            return true;
        }
        catch (error) {
            console.error(`Failed to register server ${serverConfig.name}:`, error);
    ...
    const discoverTools = async (sessionId) => {
    ...
    const executeToolCall = async (sessionId, toolName, args) => {
            ...
            while (retries <= maxRetries) {
                try {
                    ...
                    if (requiresCredentials) {
                        ...
                        const credentials = sessionManager.getToolCredentials(nextRequest.sessionId, nextRequest.toolName, toolServerId);
                        if (credentials) {
                            ...
                            const missingCredentials = toolInfo.credentialRequirements?.filter(cred => !callArgs[cred.id]);
    ...
    const getToolsWithCredentialRequirements = (sessionId) => {
    ...
    const setToolCredentials = async (sessionId, toolName, serverId, credentials) => {
        ...
        try {
            ...
            sessionManager.setToolCredentials(sessionId, toolName, serverId, credentials);
    ...
    const fetchRegistryServers = async () => {
        try {
            ...
            if (registryServers.length > 0) {
                ...
                const serverConfigs = registryServers.map(server => ({
                    id: server.id,
                    name: server.name,
                    url: server.url,
