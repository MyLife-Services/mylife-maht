...
export class SessionManager {
    constructor() {
        ...
        this.encryptionKey = Buffer.from(process.env.CREDENTIAL_ENCRYPTION_KEY ||
    ...
    setAnthropicApiKey(sessionId, apiKey) {
    ...
    getAnthropicApiKey(sessionId) {
        return this.sessions.get(sessionId)?.anthropicApiKey;
    }
    // Store credentials for a tool
    setToolCredentials(sessionId, toolName, serverId, credentials) {
        const session = this.sessions.get(sessionId);
        if (!session) {
        ...
        session.credentials = session.credentials.filter(cred => !(cred.toolName === toolName && cred.serverId === serverId));
    ...
    getToolCredentials(sessionId, toolName, serverId) {
        const session = this.sessions.get(sessionId);
        if (!session) {
            console.log(`❌ Cannot retrieve credentials: Session ${sessionId} not found`);
            return null;
        }
        const credential = session.credentials.find(cred => cred.toolName === toolName && cred.serverId === serverId);
        if (!credential) {
