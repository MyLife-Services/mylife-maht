...
async function activateBot(ctx){
	...
	ctx.body =await DigitalSelf.setActiveBot(bid)
}
/**
...
async function bot(ctx){
	const { bid, } = ctx.params
	const { DigitalSelf, } = ctx.state
	ctx.body = await DigitalSelf.getBot(bid)
}
/**
...
async function bots(ctx){
	...
	switch(ctx.method){
		case 'DELETE': // retire bot
			if(!ctx.Globals.isValidGuid(bid))
				ctx.throw(400, `missing bot id`)
			ctx.body = await DigitalSelf.retireBot(bid)
			break
		case 'POST': // create new bot
			ctx.body = await DigitalSelf.createBot(bot)
			break
		case 'PUT': // update bot
			ctx.body = await DigitalSelf.updateBot(bot)
			break
		case 'GET':
		default:
			...
			const bots = await DigitalSelf.getBots()
			let { activeBotId, greeting, } = DigitalSelf
			if(!activeBotId){
				const { id, greeting: activeGreeting } = await DigitalSelf.setActiveBot()
				activeBotId = id
...
async function botButtons(ctx){
    ...
    if(!ctx.Globals.isValidGuid(bid))
        ctx.throw(400, `missing bot id`)
    const { DigitalSelf, } = ctx.state
    const response = await DigitalSelf.botButtons(bid)
    ctx.body = response
}
/**
...
async function botOptions(ctx){
    ...
    if(!ctx.Globals.isValidGuid(bid))
        ctx.throw(400, `missing bot id`)
    const { DigitalSelf, } = ctx.state
    const response = await DigitalSelf.botOptions(bid)
    ctx.body = response
}
/**
 * Chat with the Member or System DigitalSelf's intelligence.
 * @public
 * @async
 * @param {Koa} ctx - Koa Context object
 ...
 * @property {Object[]} responses - Response messages from DigitalSelf intelligence
 */
async function chat(ctx){
	...
	const { DigitalSelf, } = ctx.state
	if(botId?.length && botId!==DigitalSelf.activeBotId)
		throw new Error(`Bot ${ botId } not currently active; chat() requires active bot`)
	const response = await DigitalSelf.chat(message, itemId, ctx.session)
	ctx.body = response
}
/**
...
async function migrateBot(ctx){
	const { bid, } = ctx.params
	const { DigitalSelf, } = ctx.state
	ctx.body = await DigitalSelf.migrateBot(bid)
}
/**
...
async function migrateChat(ctx){
	const { bid, } = ctx.params
	const { DigitalSelf, } = ctx.state
	ctx.body = await DigitalSelf.migrateChat(bid)
}
/**
...
async function retireChat(ctx){
	...
	if(!bid?.length)
		ctx.throw(400, `missing bot id`)
	const response = await DigitalSelf.retireChat(bid)
	ctx.body = response
}
/**
...
async function routine(ctx){
	...
	const { DigitalSelf, } = ctx.state
	const response = await DigitalSelf.routine(rid)
	ctx.body = response
}
/**
...
async function teams(ctx){
	const { DigitalSelf, } = ctx.state
	ctx.body = await DigitalSelf.teams()
}
/**
...
async function shadows(ctx){
	const { DigitalSelf, } = ctx.state
	const response = await DigitalSelf.shadows()
	ctx.body = response
}
/**
...
async function updateBotInstructions(ctx){
	...
	if(!bid?.length)
		ctx.throw(400, `missing bot id`)
	const { DigitalSelf, } = ctx.state
	const bot = await DigitalSelf.updateBotInstructions(bid)
	ctx.body = {
		bot,
		success: !!bot,
	}
