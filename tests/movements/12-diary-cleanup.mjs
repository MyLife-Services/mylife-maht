...
export async function play(){
    ...
    if(!entryId && !botId){
        console.log('  ✗ Cannot run — no diaryEntryId or diaryBotId in context. Run Movements 09 and 10 first.')
        return { passed: false, tally: '0/4' }
    }
    /* ── movement 1 — delete the diary entry ── */
    if(!entryId){
        sections.push({ name: 'Delete diary entry', passed: false, learned: {}, notes: 'Skipped — no diaryEntryId in context' })
    } else {
        const deleteResponse = await request(`/members/items/${ entryId }`, { method: 'DELETE', })
    ...
    if(!botId){
        sections.push({ name: 'Retire diary bot', passed: false, learned: {}, notes: 'Skipped — no diaryBotId in context' })
    } else {
