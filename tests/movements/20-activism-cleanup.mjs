...
export async function play(){
    ...
    if(!actionId && !botId){
        console.log('  ✗ Cannot run — no createdActionId or activismBotId in context. Run Movements 17 and 18 first.')
        return { passed: false, tally: '0/4' }
    }
    /* ── movement 1 — delete the action item ── */
    if(!actionId){
        sections.push({ name: 'Delete action item', passed: false, learned: {}, notes: 'Skipped — no createdActionId in context' })
    } else {
    ...
    if(!botId){
        sections.push({ name: 'Retire activism bot', passed: false, learned: {}, notes: 'Skipped — no activismBotId in context' })
    } else {
