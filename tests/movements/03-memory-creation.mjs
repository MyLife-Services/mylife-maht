...
export async function play(){
    ...
    if(!context.activeBotId){
        console.log('  ✗ Cannot run — no active bot in context. Run Movements 01 and 02 first.')
    ...
    if(!sections[0].passed){
        sections.push({ name: 'Submit fictional memory narrative', passed: false, learned: {}, notes: 'Skipped — movement 1 failed' })
        sections.push({ name: 'Verify memory in collections', passed: false, learned: {}, notes: 'Skipped — movement 1 failed' })
        return movementReport(movement.name, sections)
    }
    /* ── movement 2 — submit fictional memory, negotiate save ── */
    const m1Compelling = sections[0].learned?.compelling ?? false
    let exchangeCount = 0
    ...
    sections.push(section(
        ...
        result => {
            ...
            console.log(`    Salient coverage: ${ assessment.summary.salientCoverage } (${ (assessment.summary.salientMovement * 100).toFixed(0) }%)`)
